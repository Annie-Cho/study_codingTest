...
function combineTwoArray(first = [], second = []) {
  /** 아래와 같이 sort 사용 시
   * 시간복잡도 : O(nxlogN) = O(NlogN) : 0.021ms
   * 호호호 이런 경우 투포인터 사용하기 호호호
   * 
  const answer = [...first, ...second].sort((a, b) => a - b);
  return answer;
  */
...
const first = [1, 3, 5];
const second = [2, 3, 6, 7, 9];
console.log(combineTwoArray(first, second));
