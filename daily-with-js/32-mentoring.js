...
function findMentoring(studentCount, testResult) {
  let answer = 0;
  ...
  for (let i = 1; i <= studentCount; i++) {
    for (let j = 1; j <= studentCount; j++) {
      ...
      let count = 0;
      let mentor, mentee;
      for (let k = 0; k < testResult.length; k++) {
        for (let l = 0; l < studentCount; l++) {
          if (testResult[k][l] === i) mentor = l;
          if (testResult[k][l] === j) mentee = l;
        }
...
console.log(findMentoring(4, testResult));
